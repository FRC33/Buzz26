...
import lib.control.PathSegment;
...
public class PathBuilder {
    ...
    /**
     * Function added by FRC Team #33
     * Used to shift a list of waypoints to be relative to an origin point
     * @param w The list of waypoints to shift
     * @param origin The origin point to shift the waypoints relative to
     */
    public static void shiftWaypoints(List<Waypoint> w, Translation2d origin) {
        w.forEach((Waypoint waypoint) -> {
            waypoint.position = waypoint.position.translateBy(origin.inverse());
        });
    }

    public static void shiftWaypoints(List<Waypoint> w, double x, double y) {
        shiftWaypoints(w, new Translation2d(x, y));
    }

    /**
     * Shifts waypoints so that the first waypoint is the origin
     */
    public static void shiftWaypoints(List<Waypoint> w) {
        shiftWaypoints(w, new Translation2d(w.get(0).position));
    }
